...
export class WalletManager {
  ...
  private loadWalletsFromEnv(): void {
    const privateKeys = process.env.HYPERION_PRIVATE_KEYS?.split(',') || [];
    const addresses = process.env.HYPERION_ADDRESSES?.split(',') || [];
    const currentAddress = process.env.HYPERION_CURRENT_ADDRESS;
