...
describe('Smithery Server', () => {
  describe('configSchema', () => {
    it('should validate default configuration', () => {
      ...
      expect(config.explorerUrl).toBe('https://hyperion-testnet-explorer.metisdevops.link');
    ...
    it('should use defaults for missing optional fields', () => {
      ...
      expect(config.rpcUrl).toBe('https://custom-rpc.example.com');
      expect(config.chainId).toBe(12345);
      expect(config.networkName).toBe('Hyperion Testnet'); // default
      expect(config.currencySymbol).toBe('tMETIS'); // default
